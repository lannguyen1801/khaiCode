                                    ...
                                    <div class="table-wrapper">
                                            ...
                                            <form method="POST" action="#">
                                                <div class="modal-header">                      
                                                    <h4 class="modal-title"><b>Môn học giảng dạy</b></h4>
                                            ...
                                            <table class="table table-striped table-hover">
                                                ...
                                                <tbody>
                                                    <?php
                                                        /*$sql = "select * from monhoc m 
                                                                    join canbo c on m.macb = c.macb 
                                                                    join ctdtmh cm on cm.mamh = m.mamh
                                                                    join svctdt s on s.mact = cm.mact
                                                                    join ctdt ct on ct.mact = s.mact
                                                                    where c.tentk='".$tentk."' and trangthai='dang ky' group by m.mamh, s.mact";*/
                                                         $sql = "select * from monhoc m 
                                                                    join canbo c on m.macb = c.macb 
                                                                    join ctdtmh cm on cm.mamh = m.mamh
                                                                    join svctdt s on s.mact = cm.mact
                                                                    where c.tentk='".$tentk."' and trangthai='dang ky' group by m.mamh";
                                                        $res = mysqli_query($con,$sql);
                                                        $siso = 0;
                                                        while ($row = mysqli_fetch_array($res)){
                                                            $sql_ss="SELECT count(*) as siso from ketqua where mamh='".$row["mamh"]."'";
                                                            $result=mysqli_query($con,$sql_ss);
                                                            $siso=mysqli_fetch_assoc($result);
                                                            if ($siso['siso']>0){
                                                            echo '<tr>
                                                                    <td>'.$row["mamh"].'</td>
                                                                    <td>'.$row["tenmh"].'</td>
                                                            ...
                                                            if ($row['nhapdiem']==1){
                                                                echo'
                                                                    <a  href="nhapdiem.php?id='.$row["mamh"].'" class="adiem" title="Nhập điểm" id="'.$row["mamh"].'" data-toggle="tooltip"><i class="material-icons" >edit</i></a>';
                                                            }
                                                            echo '
                                                                    <a href="xemdiem.php?id='.$row["mamh"].'" class="viewdiem" title="Xem điểm" id="'.$row["mamh"].'" data-toggle="tooltip"><i class="material-icons" >pageview</i></a></td>
                                                                </tr>';
                                                            }
                                                        }
                                                    ?>       
